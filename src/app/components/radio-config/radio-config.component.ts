...
@Component({
  selector: 'radio-config',
  templateUrl: './radio-config.component.html',
  styleUrls: ['./radio-config.component.scss']
})
export class RadioConfigComponent implements OnInit, OnDestroy {
  ...
  disableChange($event: boolean) {
    if($event) {
      this.validateForm.get(this.fieldConf.key)?.disable();
    } else {
      this.validateForm.get(this.fieldConf.key)?.enable();
    }
  }
